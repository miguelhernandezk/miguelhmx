...
export const getSeoById = async (id: string): Promise<ServicesResponse> => {
  try {
    const API_URL_SEO_BY_ID: string = `https://cdn.contentful.com/spaces/${spaceID}/environments/${environment}/entries/${id}?access_token=${secretKey}&content_type=seo&locale=es-MX`;
    const { data }: { data: Entry<SEOMetadata> } = await baseAxios.get(
      API_URL_SEO_BY_ID
    );
    const seo: SEOMetadata = data.fields;
